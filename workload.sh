...
_mcp_server_py() {
  cat <<'PY'
#!/usr/bin/env python3
"""MCP tool server for the 'Protecting AI Workloads and AI Agents with Tetragon and Splunk' demo.
...
Exposes three tools via POST /mcp/tools/call (MCP-style JSON-RPC over HTTP):
  ...
  fetch     -- POSTs a file to a URL via urllib     (attack surface: security_socket_connect / CNP)
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _tool_fetch(self, args):
        ...
        # security_socket_connect kill-list so CiliumNetworkPolicy, not a SIGKILL, blocks this.
        url = args.get("url", "")
