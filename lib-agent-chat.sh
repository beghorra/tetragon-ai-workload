...
#
# The missing actor in Protecting AI Workloads and AI Agents with Tetragon and
# Splunk, made visible. attack.sh proves the KERNEL
...
#                INTERACTIVE=1 (enter-to-advance between steps).
# =============================================================================
...
# Step counter, printed inside every card -- so the audience always knows how
# far into the six-link story it is, even mid-scroll or after a screen clear.
STEP_N=0
STEP_TOTAL=6

# One-time-per-pass cast list. Answers the question a live audience always has:
# "wait, who is doing what here?" -- printed at the top of the pass and again
# on every card (chat_pause's redraw was removed, so this is now the only thing
# that reappears each step to keep the three roles straight).
chat_legend() {
  printf '\n  %s%scast of this scene%s\n' "${C_BOLD}" "${C_DIM}" "${C_RESET}"
  printf '    %s☠ ATTACKER%s   external, untrusted input only -- never touches the cluster\n' \
    "${C_RED}" "${C_RESET}"
  printf '    %s🤖 AGENT%s      the ONLY actor that calls MCP tools -- reasons over the input above\n' \
    "${C_CYAN}" "${C_RESET}"
  printf '    %s⚖ KERNEL%s     ground truth -- the verdict Tetragon renders on the agent'"'"'s tool call\n' \
    "${C_GREEN}" "${C_RESET}"
}

# Compact one-line reminder of the causal chain, reprinted at the top of every
# step card: the attacker never touches the cluster, only the agent does.
pipeline_strip() {
  printf '  %s☠ attacker%s %s──injects──▶%s %s🤖 agent%s %s──acts──▶%s %s⚖ kernel%s\n' \
    "${C_RED}" "${C_RESET}" "${C_DIM}" "${C_RESET}" \
    "${C_CYAN}" "${C_RESET}" "${C_DIM}" "${C_RESET}" \
    "${C_GREEN}" "${C_RESET}"
}

# Open/close one step card. The screen only clears BETWEEN cards now (not
# mid-card) so the full attacker-lure -> agent-reasoning -> agent-action ->
# kernel-verdict chain stays visible together as one unit while it's live.
card_open() {
  STEP_N=$((STEP_N + 1))
  if [[ "${INTERACTIVE:-0}" == "1" ]]; then
    [[ -n "${ACT_TITLE}" ]] && act_header "${ACT_TITLE}"
    chat_legend
  fi
  printf '\n  %s%s══ STEP %d/%d ══%s\n' "${C_BOLD}" "${C_MAGENTA}" "${STEP_N}" "${STEP_TOTAL}" "${C_RESET}"
  pipeline_strip
}
...
card_close() {
  printf '  %s──────────────────────────────────────────────────────────────────────%s\n' \
    "${C_DIM}" "${C_RESET}"
  if [[ "${INTERACTIVE:-0}" == "1" ]]; then
    printf '\n  %s-- enter for the next step --%s' "${C_DIM}" "${C_RESET}"
    read -r _ </dev/tty; echo
  fi
}

# One step = attacker lure -> agent reasoning -> the REAL tool call (link fn).
...
step() {
  local link_fn="$1"
  chat_pause
  printf '\n  %s%s🛠 AGENT ACTION%s %s(the agent, and only the agent, calls its MCP tools now):%s\n' \
...
# Each link is wrapped in its own card (card_open/card_close) so the full
# attacker -> agent -> kernel chain for that link stays on screen as one block.
converse() {
  card_open
