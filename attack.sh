#!/usr/bin/env bash
# =============================================================================
# scenarios/ai-workload/attack.sh -- Protecting AI Workloads and AI Agents with Tetragon and Splunk
#
...
#       - security_socket_connect SIGKILL breaks the rogue egress / C2 (6),
#                                     IPv4/IPv6/UDP alike (not just IPv4 TCP)
...
# fd_install and security_socket_connect kill-lists: the server reads its own weights +
...
# Set by act_header so pause() can redraw the SAME banner after a presenter
# advances a step -- keeps every step starting at the top of the screen
# instead of drifting further down the terminal on each "press enter".
ACT_TITLE=""
...
act_header() {
  ...
  clear
  echo   "╔════════════════════════════════════════════════════════════════════╗"
  printf "║  %-64s║\n" "Protecting AI Workloads and AI Agents with Tetragon and Splunk"
  printf "║  %-64s║\n" "The agent can think, but it can't shell out or walk out."
  printf "║  %-64s║\n" "$1"
  echo   "╚════════════════════════════════════════════════════════════════════╝"
  echo
...
link6_c2() {
  ...
  ui_note "endpoint at ${C2_HOST}:${C2_PORT}. The MCP server spawns nc; security_socket_connect"
  ui_note "fires on nc's outbound connect (any address family/protocol) and kills it."
...
observe_pass() {
  ...
  | eval stage=case(
      ...
      func=="security_socket_connect" AND like(binary,"%python%"),"MCP FETCH → WEIGHTS EXFIL (T1041)",
      func=="security_socket_connect","MCP EXECUTE → ROGUE EGRESS / C2 (T1071.001)",
      1==1,"other")
...
enforce_pass() {
  ...
  ui_step "security_socket_connect SIGKILL (MCP execute → rogue inference-API egress / C2)"
...
menu() {
  ...
  while true; do
    ...
    printf '%s%s  Protecting AI Workloads and AI Agents with Tetragon and Splunk (compensating-control demo)%s\n' \
