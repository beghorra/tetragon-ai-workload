# Protecting AI Workloads and AI Agents with Tetragon and Splunk — OSS Tetragon + Cilium AI Workload Demo
...
sequenceDiagram
    ...
    M->>K: nc connect()
    Note right of K: security_socket_connect · T1071.001
```
...
| `security_socket_connect` | binaries: `nc`, `wget`, `sh`, `bash`, `busybox` | Fires when `nc` or `wget` attempt an outbound connect — any address family (IPv4/IPv6) or protocol, not just IPv4 TCP | 2 (backstop), 6 |
...
| `enforce-policy.yaml` | `TracingPolicyNamespaced` scoped to `ai-workload-oss` (ENFORCE). `action: Sigkill` on `security_bprm_check`, `fd_install`, `security_socket_connect`. |
